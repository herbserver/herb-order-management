 // Automatically detect server address
        ...
        const API_URL = `${protocol}//${hostname}:3000/api`;
        ...
        function initOrderForm() {
            ...
            // Set today's date for type="date" input (YYYY-MM-DD)
            const today = new Date();
            const dd = String(today.getDate()).padStart(2, '0');
            const mm = String(today.getMonth() + 1).padStart(2, '0');
            const yyyy = today.getFullYear();

            document.querySelector('[name="date"]').value = `${yyyy}
            ...
            try {
                ...
                orders.forEach(order => {
                    html += ` <div class="order-card bg-white border rounded-xl p-4"> <div class="flex justify-between items-start"> <div> <p class="font-bold text-blue-600">${order.orderId}
                        ...
                        </p> </div> </div> <div class="mt-3 p-3 bg-gray-50 rounded-lg text-sm"> <p><strong>📍 Address:</strong> ${order.address || 'N/A'
                        }
                        ...
                        ${order.landMark ? `<p> <strong>Landmark:</strong> ${order.landMark}

                            </p> ` : ''
                        }

                        </div> <div class="mt-3 flex gap-2 flex-wrap"> <button type="button" onclick="viewOrder('${order.orderId}')" class="bg-indigo-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-indigo-600">👁️ View</button> <button type="button" onclick="openEditOrderModal('${order.orderId}')" class="bg-orange-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-orange-600">✏️ Edit</button> ${currentDeptType === 'verification'
                            ? `<button type="button" onclick="verifyAddress('${order.orderId}')" class="bg-green-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-600">✅ Verify</button>
                               <button type="button" onclick="markAsUnverified('${order.orderId}')" class="bg-yellow-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-yellow-600">⏸️ Hold</button>`
                            : `<button type="button" onclick="openDispatchModal('${order.orderId}')" class="bg-purple-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-purple-600">📦 Dispatch with Tracking</button>`
                        }
        ...
        async function loadDispatchHistory() {
            try {
                ...
                if (orders.length === 0) {
                    document.getElementById('dispatchHistoryList').innerHTML = '<p class="text-center text-gray-500 py-8">Koi orders nahi hain</p>';
                    return;
                }
                ...
                orders.forEach(order => {
                    html += ` <div class="order-card bg-white border rounded-xl p-4"> <div class="flex justify-between items-start"> <div> <p class="font-bold text-blue-600">${order.orderId}
                        ...
                        </p> <p class="text-xs px-2 py-1 rounded-full inline-block ${order.status === 'Delivered' ? 'bg-green-100 text-green-700' :
                            order.status === 'Dispatched' ? 'bg-purple-100 text-purple-700' : order.status === 'Address Verified' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'
                        }
 ...
 <p class="text-xs text-gray-500 mt-1">${order.timestamp ? new Date(order.timestamp).toLocaleDateString() : ''
        ...
        function switchAdminTab(tab) {
            ...
            document.getElementById(`admin${tab.charAt(0).toUpperCase() + tab.slice(1)
                }

                Tab`).classList.remove('hidden');

            document.getElementById(`adminTab${tab.charAt(0).toUpperCase() + tab.slice(1)
                }
        ...
        async function updateDepartmentFull() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    showMessage(`✅ Department fully updated ! New ID: ${newId}
        ...
        async function exportAllOrders() {
            try {
                ...
                let csv = 'Order ID,Employee,Customer,Tel,Address,PIN,State,Total,COD,Status,Courier,Tracking,Date\n';

                orders.forEach(o => {
                    const t = o.tracking || {}

                        ;
                    csv += `"${o.orderId}", "${o.employee}", "${o.customerName}", "${o.telNo}", "${(o.address || '').replace(/" /g, '""')
                        }
        ...
        async function viewOrder(orderId) {
            try {
                ...
                </h4><div class="inline-block px-4 py-1 rounded-full text-sm font-semibold ${order.status === 'Delivered' ? 'bg-green-400 text-green-900' :
                        order.status === 'Dispatched' ? 'bg-purple-400 text-purple-900' : order.status === 'Address Verified' ? 'bg-blue-300 text-blue-900' : 'bg-yellow-400 text-yellow-900'
                    }
            ...
            </span></p>${order.altNo ? `<p class="flex items-center gap-2"><span class="font-semibold text-gray-600">Alt:</span><span class="text-gray-800 font-mono">${order.altNo}

                </span></p> ` : ''
                    }

            <p class="flex items-center gap-2"><span class="font-semibold text-gray-600">Type:</span><span class="px-2 py-1 bg-blue-100 text-blue-700 rounded-lg text-xs font-semibold">${order.orderType || 'NEW'
                    }

            </span></p></div></div><!-- Address with Icon --><div class="bg-white border-2 border-gray-100 p-5 rounded-2xl shadow-md hover:shadow-lg transition-shadow"><div class="flex items-center gap-3 mb-3"><div class="w-12 h-12 bg-gradient-to-br from-red-400 to-pink-600 rounded-full flex items-center justify-center text-white text-xl">📍 </div><h5 class="text-lg font-bold text-gray-800">Delivery Address</h5></div><p class="text-gray-700 leading-relaxed">${fullAddress}
        ...
        async function loadVerificationUnverified() {
            try {
                ...
                if (orders.length === 0) {
                    document.getElementById('verificationUnverifiedList').innerHTML = '<p class="text-center text-gray-500 py-8">Koi unverified/hold orders nahi hain</p>';
                    return;
                }
        ...
        async function markAsUnverified(orderId) {
            if (!confirm('Kya aap is order ko Unverified/Hold list mein daalna chahte hain?')) return;
            try {
                const res = await fetch(`${API_URL}/orders/${orderId}/status`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ status: 'On Hold', employee: currentUser.name })
                });
                const data = await res.json();
                if (data.success) {
                    ...
                    // Logic to refresh active tab if needed
                    ...
                    msgDiv.className = 'fixed bottom-4 right-4 bg-yellow-500 text-white px-4 py-2 rounded-lg shadow-lg z-50 animate-fadeIn';
                    msgDiv.innerHTML = 'Order moved to Unverified/Hold';
        ...
        async function loadVerificationHistory() {
            try {
                ...
                const startDate = document.getElementById('verificationHistoryStartDate').value;
                const endDate = document.getElementById('verificationHistoryEndDate').value;
                ...
                if (orders.length === 0) {
                    document.getElementById('verificationHistoryList').innerHTML = '<p class="text-center text-gray-500 py-8">Koi orders nahi hain is date range mein</p>';
                    return;
                }

                let html = '';
                orders.forEach(order => {
                    html += `<div class="order-card bg-white border rounded-xl p-4">
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="font-bold text-blue-600">${order.orderId}</p>
                                <p class="text-gray-800 font-medium">${order.customerName}</p>
                            ...
                            <div class="text-right">
                                <p class="text-lg font-bold text-red-600">₹${order.total}</p>
                                <p class="text-xs px-2 py-1 rounded-full inline-block ${order.status === 'Delivered' ? 'bg-green-100 text-green-700' : order.status === 'Dispatched' ? 'bg-purple-100 text-purple-700' : order.status === 'Address Verified' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'}">
                                    ${order.status}
                                </p>
                                <p class="text-xs text-gray-500 mt-1">${order.timestamp ? new Date(order.timestamp).toLocaleDateString() : ''}</p>
                            </div>
                        </div>
                        <div class="mt-3">
                            <button type="button" onclick="viewOrder('${order.orderId}')" class="bg-indigo-500 text-white px-3 py-1.5 rounded-lg text-xs hover:bg-indigo-600">👁️ View</button>
