...
function renderDispatchCardModern(o) {
    ...
    const vRemark = o.verificationRemark?.text || '';
    ...
    <div class="bg-gradient-to-br from-white to-orange-50/30 border-2 border-orange-200 rounded-2xl p-5 hover:shadow-2xl hover:border-orange-400 transition-all duration-300 relative overflow-hidden group">
        ...
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl p-4 mb-4 shadow-sm">
            ...
            ${suggestedBy ? `<p class="text-xs text-blue-500 ml-8 mt-1">Suggested By: ${suggestedBy}</p>` : ''}
        </div>
        ` : ''}
        
        ${eRemark ? `
        <div class="bg-gradient-to-r from-rose-50 to-pink-50 border-2 border-rose-200 rounded-xl p-4 mb-4 shadow-sm">
            <div class="flex items-start gap-3">
                <span class="text-xl">💬</span>
                <div>
                   <p class="text-[10px] font-black text-rose-400 uppercase tracking-widest mb-1">Employee Remark</p>
                   <p class="text-sm text-rose-900 font-bold italic">"${eRemark}"</p>
                </div>
            </div>
        </div>
        ` : ''}
        ...
        ${vRemark ? `
        <div class="bg-gradient-to-r from-yellow-50 to-orange-50 border-2 border-yellow-200 rounded-xl p-4 mb-4 shadow-sm">
            <div class="flex items-start gap-3">
                <span class="text-xl">📝</span>
                <div>
                   <p class="text-[10px] font-black text-yellow-500 uppercase tracking-widest mb-1">Verification Note</p>
                   <p class="text-sm text-yellow-900 font-bold leading-relaxed">${vRemark}</p>
                </div>
            </div>
        </div>
        ` : ''}
