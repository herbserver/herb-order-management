...
function filterDeptOrdersHeader(query) {
    ...
    if (q.startsWith('#')) {
        if (isDeliveryDepartment() && typeof window.handleDeliveryCommand === 'function' && window.handleDeliveryCommand(q)) {
            return;
        }
        if (typeof window.handleVerificationCommand === 'function' && window.handleVerificationCommand(q)) {
            return;
        }
        if (typeof window.handleDispatchCommand === 'function' && window.handleDispatchCommand(q)) {
            return;
        }
    }
