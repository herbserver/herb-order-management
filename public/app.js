...
function getDeliveryTabFromHash() {
    return normalizeDeliveryTab(window.location.hash.replace(/^#/, ''));
}
...
function updateDeliveryTabHash(tab, replaceOnly = false) {
    const normalizedTab = normalizeDeliveryTab(tab);
    const nextHash = `#${normalizedTab}`;
...
function switchDeliveryTab(tab, syncHash = true) {
    ...
    if (syncHash) {
        updateDeliveryTabHash(tab);
    }
}

window.addEventListener('hashchange', function () {
    if (currentDeptType !== 'delivery') return;
    const requestedTab = getDeliveryTabFromHash();
    if (getActiveDeliveryTab() === requestedTab) return;
    switchDeliveryTab(requestedTab, false);
});


// Load Out for Delivery Orders
async function loadDeliveryOrders() {
    return loadOFDOrders();
}
...
function renderDeliveryDeptCard(order, fallbackColor) {
    if (typeof renderDeliveryCardModern === 'function') {
        return renderDeliveryCardModern(order);
    }
    if (typeof renderOrderCard === 'function') {
        return renderOrderCard(order, fallbackColor);
    }
    return '';
}
...
function formatInventoryInputDate(date) {
...
function formatInventoryDisplayDate(dateText) {
    if (!dateText) return '';
...
function renderInventorySummaryCards(summary, basisLabel) {
    const container = document.getElementById('inventorySummaryGrid');
    ...
    const cards = [
        ...
        },
        {
            ...
            tone: 'from-teal-50 to-cyan-50 border-teal-200 text-teal-700'
        },
        {
            label: 'Active Days',
        ...
        {
            label: basisLabel || 'Unique Products',
    ...
    container.innerHTML = cards.map((card) => `
        <div class="bg-gradient-to-br ${card.tone} border rounded-2xl p-5 shadow-sm">
            <p class="text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">${inventoryEscapeHtml(card.label)}</p>
            <div class="mt-3 flex items-end justify-between gap-3">
                <p class="text-3xl font-black text-slate-800">${Number(card.value || 0).toLocaleString()}</p>
                <span class="text-xs font-bold ${card.tone.split(' ').pop()}">${summary.emptyItemOrders ? `${summary.emptyItemOrders} empty items` : 'Ready'}</span>
            </div>
        </div>
    `).join('');
}

function renderInventoryTopProducts(products) {
    const container = document.getElementById('inventoryTopProducts');
    if (!container) return;

    if (!products || products.length === 0) {
        container.innerHTML = '<div class="text-sm text-slate-400 py-8 text-center">No products found in this range.</div>';
        return;
    }
    ...
    container.innerHTML = products.slice(0, 10).map((product, index) => `
        <div class="flex items-center justify-between gap-3 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
            <div class="min-w-0">
                <div class="flex items-center gap-2">
                    <span class="text-[11px] font-black text-slate-400 w-6">${index + 1}.</span>
                    <p class="font-bold text-slate-800 truncate">${inventoryEscapeHtml(product.name)}</p>
                </div>
                <p class="text-[11px] text-slate-500 ml-8">${Number(product.orderCount || 0).toLocaleString()} orders</p>
            </div>
            <div class="text-right">
                <p class="text-lg font-black text-teal-700">${Number(product.quantity || 0).toLocaleString()}</p>
                <p class="text-[11px] font-semibold text-slate-400">units</p>
            </div>
        </div>
    `).join('');
}

function renderInventoryDailyTable(daily) {
    const container = document.getElementById('inventoryDailyTable');
    if (!container) return;

    if (!daily || daily.length === 0) {
        container.innerHTML = '<div class="text-sm text-slate-400 py-8 text-center">No day-wise inventory found for this range.</div>';
        return;
    }
    ...
    container.innerHTML = `
        <table class="min-w-full text-sm">
            <thead>
                <tr class="border-b border-slate-200 text-left text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">
                    <th class="py-3 pr-4">Date</th>
                    <th class="py-3 pr-4 text-right">Orders</th>
                    <th class="py-3 pr-4 text-right">Units</th>
                    <th class="py-3 pr-4 text-right">Products</th>
                    <th class="py-3">Top Items</th>
                </tr>
            </thead>
            <tbody>
                ${daily.map((day) => `
                    <tr class="border-b border-slate-100 align-top">
                        <td class="py-3 pr-4">
                            <p class="font-bold text-slate-800">${inventoryEscapeHtml(formatInventoryDisplayDate(day.date))}</p>
                            <p class="text-[11px] text-slate-400">${inventoryEscapeHtml(day.date)}</p>
                        </td>
                        <td class="py-3 pr-4 text-right font-semibold text-slate-700">${Number(day.ordersCount || 0).toLocaleString()}</td>
                        <td class="py-3 pr-4 text-right font-black text-teal-700">${Number(day.totalUnits || 0).toLocaleString()}</td>
                        <td class="py-3 pr-4 text-right font-semibold text-slate-700">${Number(day.uniqueProducts || 0).toLocaleString()}</td>
                        <td class="py-3">
                            <div class="flex flex-wrap gap-2">
                                ${(day.topProducts || []).map((product) => `
                                    <span class="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-[11px] font-bold text-slate-600">
                                        ${inventoryEscapeHtml(product.name)} x${Number(product.quantity || 0).toLocaleString()}
                                    </span>
                                `).join('')}
                            </div>
                        </td>
...
function renderInventoryDailyCards(daily) {
    const container = document.getElementById('inventoryDailyCards');
    if (!container) return;

    if (!daily || daily.length === 0) {
        container.innerHTML = '<div class="text-sm text-slate-400 py-8 text-center">No product breakdown found for this range.</div>';
        return;
    }
    ...
    container.innerHTML = daily.map((day) => `
        <div class="rounded-2xl border border-slate-100 bg-slate-50 p-4 shadow-sm">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <h5 class="text-lg font-black text-slate-800">${inventoryEscapeHtml(formatInventoryDisplayDate(day.date))}</h5>
                    <p class="text-xs font-semibold text-slate-400">${inventoryEscapeHtml(day.date)}</p>
                </div>
                <div class="flex flex-wrap gap-3 text-right">
                    <div class="rounded-xl bg-white px-4 py-2 border border-slate-200">
                        <p class="text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">Orders</p>
                        <p class="text-lg font-black text-slate-800">${Number(day.ordersCount || 0).toLocaleString()}</p>
                    </div>
                    <div class="rounded-xl bg-white px-4 py-2 border border-teal-200">
                        <p class="text-[11px] font-black uppercase tracking-[0.18em] text-teal-500">Units</p>
                        <p class="text-lg font-black text-teal-700">${Number(day.totalUnits || 0).toLocaleString()}</p>
                    </div>
                    <div class="rounded-xl bg-white px-4 py-2 border border-violet-200">
                        <p class="text-[11px] font-black uppercase tracking-[0.18em] text-violet-500">Products</p>
                        <p class="text-lg font-black text-violet-700">${Number(day.uniqueProducts || 0).toLocaleString()}</p>
                    </div>
                </div>
            </div>
            <div class="mt-4 flex flex-wrap gap-2">
                ${(day.products || []).map((product) => `
                    <span class="inline-flex items-center rounded-full bg-white border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-700">
                        ${inventoryEscapeHtml(product.name)} x${Number(product.quantity || 0).toLocaleString()}
                    </span>
                `).join('')}
            </div>
...
function renderAdminInventory(data) {
    ...
    const basisLabel = data.basisLabel || 'Dispatch Date';
    const rangeLabel = `${formatInventoryDisplayDate(data.startDate)} to ${formatInventoryDisplayDate(data.endDate)}`;

    const basisEl = document.getElementById('inventoryBasisLabel');
    const dateRangeEl = document.getElementById('inventoryDateRangeLabel');
    const metaEl = document.getElementById('inventoryRangeMeta');

    if (basisEl) basisEl.textContent = basisLabel;
    if (dateRangeEl) dateRangeEl.textContent = rangeLabel;
    if (metaEl) {
        metaEl.textContent = `${rangeLabel} • ${Number(summary.totalUnits || 0).toLocaleString()} units • ${Number(summary.totalOrders || 0).toLocaleString()} orders`;
    }

    renderInventorySummaryCards(summary, 'Unique Products');
    renderInventoryTopProducts(data.products || []);
    renderInventoryDailyTable(data.daily || []);
    renderInventoryDailyCards(data.daily || []);
}

window.setInventoryQuickRange = function (range) {
    const startInput = document.getElementById('inventoryStartDate');
    const endInput = document.getElementById('inventoryEndDate');
    ...
    startInput.value = formatInventoryInputDate(start);
    endInput.value = formatInventoryInputDate(today);
    loadAdminInventory();
};

window.resetAdminInventoryFilters = function () {
    const basisInput = document.getElementById('inventoryBasis');
    const startInput = document.getElementById('inventoryStartDate');
    const endInput = document.getElementById('inventoryEndDate');
    const today = new Date();
    ...
    if (basisInput) basisInput.value = 'dispatched';
    if (startInput) startInput.value = formatInventoryInputDate(monthStart);
    if (endInput) endInput.value = formatInventoryInputDate(today);

    loadAdminInventory();
};

window.loadAdminInventory = async function () {
    const basisInput = document.getElementById('inventoryBasis');
    const startInput = document.getElementById('inventoryStartDate');
    const endInput = document.getElementById('inventoryEndDate');
    if (!basisInput || !startInput || !endInput) return;

    const today = new Date();
    const monthStart = new Date(today);
    monthStart.setDate(1);

    if (!startInput.value) startInput.value = formatInventoryInputDate(monthStart);
    if (!endInput.value) endInput.value = formatInventoryInputDate(today);
    if (!basisInput.value) basisInput.value = 'dispatched';

    const dailyTable = document.getElementById('inventoryDailyTable');
    const topProducts = document.getElementById('inventoryTopProducts');
    const dailyCards = document.getElementById('inventoryDailyCards');

    if (dailyTable) dailyTable.innerHTML = '<div class="text-sm text-slate-400 py-8 text-center">Loading inventory data...</div>';
    if (topProducts) topProducts.innerHTML = '<div class="text-sm text-slate-400 py-8 text-center">Loading products...</div>';
    if (dailyCards) dailyCards.innerHTML = '<div class="text-sm text-slate-400 py-8 text-center">Loading day-wise breakdown...</div>';

    try {
        const params = new URLSearchParams({
            startDate: startInput.value,
            endDate: endInput.value,
            basis: basisInput.value,
            _t: Date.now().toString()
        });

        const res = await fetch(`${API_URL}/admin/inventory-summary?${params.toString()}`);
        const data = await res.json();
        ...
        const message = inventoryEscapeHtml(error.message || 'Unable to load inventory data');
        if (dailyTable) dailyTable.innerHTML = `<div class="text-sm text-rose-500 py-8 text-center">${message}</div>`;
        if (topProducts) topProducts.innerHTML = `<div class="text-sm text-rose-500 py-8 text-center">${message}</div>`;
        if (dailyCards) dailyCards.innerHTML = `<div class="text-sm text-rose-500 py-8 text-center">${message}</div>`;
    }
};
...
function filterDeptOrdersHeader(query) {
    ...
    if (q.startsWith('#')) {
        ...
        if (currentDeptType === 'delivery' && typeof window.handleDeliveryCommand === 'function' && window.handleDeliveryCommand(q)) {
            return;
        }
    }
    ...
    if (currentDeptType === 'verification') {
        ...
        const inputs = ['onWaySearchInput', 'ofdSearchInput', 'deliveredSearchInput', 'rtoSearchInput'];
        inputs.forEach(id => {
            const el = document.getElementById(id);
            if (el) el.value = q;
        });
        ...
        if (activeTab) {
            if (activeTab.id === 'deptTabRequests') loadDeliveryRequests();
            else if (activeTab.id === 'deptTabOnWay') loadOnWayOrders();
            else if (activeTab.id === 'deptTabOFD') loadOFDOrders();
            else if (activeTab.id === 'deptTabDelivered') loadDeliveredOrders();
            else if (activeTab.id === 'deptTabRTO') loadRTOOrders();
        }
    }
}
...
async function loadWAConversations() {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) { waAllConversations = data.conversations; renderWAConversations(data.conversations); }
